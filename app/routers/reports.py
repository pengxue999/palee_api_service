...
@router.get("/scholarship-students")
def get_scholarship_students_report(
    scholarship: Optional[str] = Query(None, description="ສະຖານະທຶນ: 'ໄດ້ຮັບທຶນ' | 'ບໍ່ໄດ້ຮັບທຶນ' (default: ໄດ້ຮັບທຶນ)"),
    subject_id: Optional[str] = Query(None, description="ລະຫັດວິຊາ (optional)"),
    level_id: Optional[str] = Query(None, description="ລະຫັດລະດັບ/ຊັ້ນຮຽນ (optional)"),
    db: Session = Depends(get_db),
):
    """
    ລາຍງານນັກຮຽນທຶນ — ສະແດງນັກຮຽນຕາມສະຖານະທຶນ ພ້ອມວິຊາທີ່ໄດ້ຮັບທຶນ

    - scholarship: ກັ່ນຕອງຕາມສະຖານະທຶນ (default 'ໄດ້ຮັບທຶນ')
    - subject_id: ກັ່ນຕອງຕາມວິຊາ
    - level_id: ກັ່ນຕອງຕາມຊັ້ນຮຽນ/ລະດັບ
    """
    result = svc.get_scholarship_report(
        db, scholarship=scholarship, subject_id=subject_id, level_id=level_id
    )
    return success_response(result, "ດຶງຂໍ້ມູນລາຍງານນັກຮຽນທຶນສຳເລັດ")


@router.get("/scholarship-students/export")
def export_scholarship_students_report(
    scholarship: Optional[str] = Query(None, description="ສະຖານະທຶນ (optional)"),
    subject_id: Optional[str] = Query(None, description="ລະຫັດວິຊາ (optional)"),
    level_id: Optional[str] = Query(None, description="ລະຫັດລະດັບ/ຊັ້ນຮຽນ (optional)"),
    format: str = Query("excel", description="ຮູບແບບໄຟລ໌: csv ຫຼື excel"),
    ...
    """Export ລາຍງານນັກຮຽນທຶນເປັນ CSV ຫຼື Excel"""
    result = svc.export_scholarship_report(
        db,
        scholarship=scholarship,
        subject_id=subject_id,
        level_id=level_id,
